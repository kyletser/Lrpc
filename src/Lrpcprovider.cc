...
class RpcResponseClosure : public google::protobuf::Closure {
public:
    RpcResponseClosure(LrpcProvider* provider,
                       muduo::net::TcpConnectionPtr conn,
                       google::protobuf::Message* request,
                       google::protobuf::Message* response)
        : provider_(provider), conn_(std::move(conn)), request_(request), response_(response) {}
...
void LrpcProvider::Run() {
    ...
    int port = atoi(LrpcApplication::GetInstance().GetConfig().Load("rpcserverport").c_str());
    ...
    for (auto &sp : service_map) {
        ...
        for (auto &mp : sp.second.method_map) {
            ...
            std::string provider_path = method_path + "/node-";
            zkclient.Create(provider_path.c_str(), method_path_data, strlen(method_path_data), ZOO_EPHEMERAL | ZOO_SEQUENCE);
        }
    }
...
void LrpcProvider::OnMessage(const muduo::net::TcpConnectionPtr &conn, muduo::net::Buffer *buffer, muduo::Timestamp receive_time) {
     // 循环处理缓冲区，解决粘包问题
    while (buffer->readableBytes() >= 4) {
        ...
        if (total_len < 4 || header_len > total_len - 4) {
            LOG(ERROR) << "invalid rpc packet length";
            conn->shutdown();
            return;
        }
        ...
        // 4. 业务逻辑处理
        if (!LrpcHeader.ParseFromString(rpc_header_str)) {
            LOG(ERROR) << "header parse error";
            return;
        }
        
        std::string service_name = LrpcHeader.service_name();
        std::string method_name = LrpcHeader.method_name();
        ...
        google::protobuf::Closure *done = new RpcResponseClosure(this, conn, request, response);
        service->CallMethod(method, nullptr, request, response, done);
    }
}

// 发送RPC响应给客户端
void LrpcProvider::SendRpcResponse(const muduo::net::TcpConnectionPtr &conn, google::protobuf::Message *response) {
