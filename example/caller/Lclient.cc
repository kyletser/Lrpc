...
void send_request(int thread_id, std::atomic<int> &success_count, std::atomic<int> &fail_count,int requests_per_thread) {
    ...
    Kuser::UserServiceRpc_Stub stub(new LrpcChannel(false));
    ...
    Lrpccontroller controller;  // 创建控制器对象，用于处理 RPC 调用过程中的错误
    for (int i = 0; i < requests_per_thread; ++i) {
        controller.Reset();

        // 调用远程的 Login 方法,通过 stub 对象发送 RPC 请求
